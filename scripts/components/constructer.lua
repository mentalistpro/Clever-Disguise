--local cooking = require("constructing")
...
local Constructer = Class(function(self, inst)
	self.inst = inst
	self.cooking = false
	self.done = false
    ...
    self.cooktimemult = 1
	self.product = nil
    self.spoiledproduct = "mermthrone"
	self.task = nil
	self.targettime = nil
...
function Constructer:OnRemoveFromEntity()
...
local function dostew(inst)
	local stewercmp = inst.components.constructer
	stewercmp.task = nil
...
function Constructer:IsCooking()
    return not self.done and self.targettime ~= nil
end

function Constructer:IsDone()
	return self.done
end
...
function Constructer:StartCooking()
...
function Constructer:StopCooking(reason)
...
function Constructer:OnLoad(data)
...
function Constructer:GetDebugString()
...
function Constructer:Harvest(harvester)
	if self.done then
		...
		if self.product ~= nil then
			local loot = SpawnPrefab("mermthrone")
			if loot ~= nil then
...
function Constructer:LongUpdate(dt)
